...
def test_DecisionTreeClassifier(*data):
    ...
    clf = DecisionTreeClassifier(max_depth=13,splitter='best')
    clf.fit(X_train, y_train)
    export_graphviz(clf, 'tree')
...
def test_DecisionTreeClassifier_depth(*data,maxdepth):
    ...
    ax.plot(depths,training_scores,label="traing score",marker='o',color='#000000')
    ax.plot(depths,testing_scores,label="testing score",marker='*',color='#000000')
...
def test_cross_val_score(X,y):
    ...
    :return: None
    '''
    num_folds = np.arange(1, 11,step=1)
    result1=cross_val_score(DecisionTreeClassifier(max_depth=13,splitter='best'),X,y,cv=10) # 使用 LinearSVC 作为分类器
    print("Cross Val Score is:",result1)
    print("Average Cross Val Score is:",sum(result1)/len(result1))

    result2 = cross_val_score(DecisionTreeClassifier(max_depth=13, splitter='best'), X, y, cv=10,scoring='f1')  # 使用 LinearSVC 作为分类器
    print("F1 Score is:", result2)
    print("Average F1 Score is:", sum(result2) / len(result2))

    result3 = cross_val_score(DecisionTreeClassifier(max_depth=13, splitter='best'), X, y, cv=10,scoring='precision')  # 使用 LinearSVC 作为分类器
    print("Precision Score is:", result3)
    print("Average Precision Score is:", sum(result3) / len(result3))

    result4 = cross_val_score(DecisionTreeClassifier(max_depth=13, splitter='best'), X, y, cv=10,
    ...
    ax.set_title("Decision-Making Tree Classifier ",font)
