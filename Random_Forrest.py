...
def test_cross_val_score(X,y):
    ...
    result1=cross_val_score(ensemble.RandomForestClassifier(bootstrap=True,criterion='gini',max_depth=50,n_estimators=142),X,y,cv=10) # 使用 LinearSVC 作为分类器
    print("Cross Val Score is:",result1)
    print("Average Cross Val Score is:",sum(result1)/len(result1))

    result2 = cross_val_score(ensemble.RandomForestClassifier(bootstrap=True,criterion='gini',max_depth=50,n_estimators=142), X, y, cv=10,scoring='f1')  # 使用 LinearSVC 作为分类器
    print("F1 Score is:", result2)
    print("Average F1 Score is:", sum(result2) / len(result2))

    result3 = cross_val_score(ensemble.RandomForestClassifier(bootstrap=True,criterion='gini',max_depth=50,n_estimators=142), X, y, cv=10,scoring='precision')  # 使用 LinearSVC 作为分类器
    print("Precision Score is:", result3)
    print("Average Precision Score is:", sum(result3) / len(result3))

    result4 = cross_val_score(ensemble.RandomForestClassifier(bootstrap=True,criterion='gini',max_depth=50,n_estimators=142), X, y, cv=10,
    ...
    ax.set_title("Random Forest Classifier",font)
...
if __name__=='__main__':
    ...
    y_pred, y_test=test_RandomForestClassifier(X_train, X_test, y_train, y_test)  # 调用 test_RandomForestClassifier
